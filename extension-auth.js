...
class ExtensionAuth {
  ...
  async backupMacros() {
    ...
    try {
      ...
      // Also try to store on website via API
      try {
        const response = await fetch(`${this.apiBase}/backup-simple-final`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${this.authToken || 'demo-token'}`
          },
        ...
        console.log('API backup failed, using local storage only:', apiError);
      }
    ...
    try {
      ...
      return new Promise((resolve) => {
        chrome.tabs.create({
          ...
          // Listen for restore message from website
          chrome.runtime.onMessage.addListener(function listener(message, sender, sendResponse) {
            if (message.type === 'RESTORE_SUCCESS' && message.userId === this.currentUser.id) {
              ...
              if (message.backup.macros) {
                chrome.storage.sync.set({ macros: message.backup.macros });
              }
              if (message.backup.folders) {
                chrome.storage.sync.set({ folders: message.backup.folders });
              }
              if (message.backup.macroStats) {
                chrome.storage.sync.set({ macroStats: message.backup.macroStats });
              }
              if (message.backup.settings) {
                chrome.storage.sync.set({ settings: message.backup.settings });
              }
